...
[The file here](./testdata/10.txt) is intelligible (somewhat) when CBC decrypted against "YELLOW SUBMARINE" with an IV
...
fn cbc_decrypt(cipher: Vec<u8>, key: &Vec<u8>, iv: Vec<u8>) -> Vec<u8> {
    vec![0u8]
}
...
fn test_cbc_decrypt() {
    ...
    assert_eq!(
        cbc_decrypt(vec![0u8], &key, std::iter::repeat(0u8).take(key.len() as usize).collect()),
        vec![1u8]
    )
}